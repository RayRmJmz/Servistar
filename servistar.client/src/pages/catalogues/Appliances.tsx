...
import useGetAppliances from "../../hooks/catalogues/useGetAppliances";
import { appliancesAdapater } from "../../adapters";
import { useState } from "react";
import AddCircleIcon from "@mui/icons-material/AddCircle";
import AddEditApplianceModal from "../../components/Appliance/AddEditApplienceModal";
import { FormProviderCreateAppliance } from "../../utils";
...
export default function Appliances() {
  ...
  const { data: appliances, isLoading, refetch } = useGetAppliances();
  const rowsAppliance = appliances ? appliancesAdapater(appliances) : [];
    ...
    <Container>
      ...
      >
        <Grid item container xs={8} sm={6} md={4} lg={3} xl={3}>
          <MuiTitle title={LABELS.APPLIANCES} />
        </Grid>
        <Grid
          item
          spacing={2}
        ...
        >
          <MuiTooltip messageTooltip={`${LABELS.ADD} ${LABELS.APPLIANCE}`}>
            <Button
              variant="contained"
      ...
      <Card>
        <Grid>
          <DataGrid
            columns={dataColumns}
            rows={rowsAppliance ? rowsAppliance : []}
            sx={{
              height: 500,
            }}
            loading={isLoading}
      ...
      <FormProviderCreateAppliance>
        <AddEditApplianceModal
